...
class SkillCsvWriterTests(unittest.TestCase):
    def run_writer(self, directory: Path, payload: dict, skill: str) -> subprocess.CompletedProcess[str]:
        input_path = directory / "register-data.json"
        output_path = directory / "artifacts" / "register.csv"
        input_path.write_text(json.dumps(payload), encoding="utf-8")
        script = ROOT / "source" / "skills" / skill / "scripts" / "write-register-csv.ps1"
    ...
    def test_writers_round_trip_commas_quotes_and_embedded_newlines(self) -> None:
        for skill in ("evidence-first-report", "editorial-data-storytelling"):
            with self.subTest(skill=skill), tempfile.TemporaryDirectory() as temp_dir:
                ...
                result = self.run_writer(directory, payload, skill)
    ...
    def test_writer_rejects_mismatched_and_whitespace_only_rows(self) -> None:
        ...
        for payload in cases:
            with self.subTest(payload=payload), tempfile.TemporaryDirectory() as temp_dir:
                result = self.run_writer(Path(temp_dir), payload, "evidence-first-report")
