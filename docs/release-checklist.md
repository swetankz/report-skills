...
- [ ] A model-free prompt-input probe runs from a fresh external system-temporary directory and confirms that the pinned model plus canonical isolation controls returns the expected prompt shape, nonempty developer text, and exact sentinel without multi-agent developer context before evaluation tokens can be spent. Its method, schema, and normalized developer-context SHA-256 are recorded in the execution profile and identity-bound across stages, excluding volatile message IDs, metadata, and the temporary environment-context item. Every release-defining call disables the `multi_agent` and `multi_agent_v2` feature paths, sets `agents.enabled=false`, rejects profiles plus noncanonical or compound overrides, and passes a pre-invocation exact-control check. Task Git discovery is fenced outside the candidate repository, and regression tests cover interruption, immutable-input mutation, output copyback, cleanup, and parent-repository access.
- [ ] Optional Graphify output is used only for local navigation or impact analysis. Its adapters, hooks, graph, caches, and reports are untracked, excluded from model workspaces and release artifacts, absent from receipts, and never used to waive or satisfy a required check.
...
- [ ] `.zcode-plugin/plugin.json` declares the target version and matches `pyproject.toml`.
- [ ] `marketplace.json` lists the `report-skills` plugin at the target version.
