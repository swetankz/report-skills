...
Write every register with a CSV serializer or equivalent standards-compliant escaping; never construct rows by joining field strings with commas. Quote fields containing a comma, double quote, carriage return, or line break, and double embedded quotes. Re-open each written CSV with a parser: use a strict UTF-8 CSV parser on the final bytes and explicitly compare every logical row's field count with the header's field count. Require a nonempty header and exactly the header's field count in every logical record; require every data row to have exactly the header's column count. In Python, use `csv.reader(..., strict=True)` and compare `len(row)` to `len(header)`. PowerShell `ConvertFrom-Csv` alone is not a sufficient width check because extra fields can be absorbed into the final property. Do not validate CSV by splitting physical lines or commas. Treat any mismatch as a blocking artifact error: do not draft from the malformed register or mark the package ready. After a repair, reopen and validate the final bytes again with the strict parser before reporting success.
