...
def trigger_task_prompt(query: str) -> str:
    ...
    return (
        ...
        "Use the declared invocation policy before any SKILL.md description as the "
        "pre-activation evidence; only a policy-eligible skill's declared description may then "
        "inform applicability. A skill directory or filename is never enough to infer "
        "applicability. If filesystem "
        "discovery finds a skill whose policy is not already known, inspect its "
        "agents/openai.yaml first and separately, before any SKILL.md access. When that "
        "metadata declares allow_implicit_invocation: false and the request lacks the "
        "required exact invocation token, reject the skill immediately and never open its "
        "SKILL.md, including its frontmatter. Only for a skill that remains eligible under "
        "the invocation policy may you inspect SKILL.md frontmatter through the exact closing "
        "delimiter before deciding; do not read the body. If a metadata or bounded-frontmatter "
        "inspection is declined, fails, or cannot be bounded exactly, fail closed: do not "
        "invoke the unknown skill and do not open any more of SKILL.md. Never use a line-count "
        "preview such as head, Get-Content -Head, Get-Content -TotalCount, Select-Object -First, "
        "or sed -n, and never use a whole-file read as a metadata or frontmatter fallback. Do "
        "not inspect or invoke a skill merely "
